...
import { getStoreShippedOrders, cancelOrder, changeStatus } from "../../../redux/apiRequest";
...
const ShippedOrders = () => {
  ...
  useEffect(() => {
    try {
        getStoreShippedOrders(user?.accessToken, dispatch, axiosJWT);
    } catch (error) {
  ...
  const handleChangeStatus = (orderId) => {
    try {
      ...
      changeStatus(user?.accessToken, dispatch, orderId, status, axiosJWT);
    } catch (error) {
      console.error("Error canceling order:", error);
    }
  };
      ...
      {orders.map((order) => (
        ...
        >
          ...
          <button
              ...
              onClick={(e) =>{e.stopPropagation();  handleChangeStatus(order._id)}}
            >
              Confirm Delivered
            </button>

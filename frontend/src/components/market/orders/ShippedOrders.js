...
import { getShippedOrders, confirmOrder } from "../../../redux/apiRequest";
...
const ShippedOrders = () => {
  ...
  useEffect(() => {
    try {
      getShippedOrders(user?.accessToken, dispatch, axiosJWT);
    } catch (error) {
  ...
  const handleConfirmReceived = (orderId) => {
    try {
      confirmOrder(user?.accessToken, dispatch, orderId, axiosJWT);
    } catch (error) {
          ...
          <div className="flex justify-between items-center mb-2">
            ...
            <div className="flex space-x-2">
              {order.status === "delivered" && !order.sellerConfirmed ? (
                <button
                  className="px-4 py-1 rounded-lg border border-green-500 text-green-500 hover:bg-green-300"
                  onClick={(e) =>{e.stopPropagation();  handleConfirmReceived(order._id)}}
                >
                  Confirm Received
                </button>
              ) : (
